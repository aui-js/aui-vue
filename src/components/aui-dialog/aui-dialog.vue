<template name="aui-dialog">
	<div class="aui-dialog" v-if="SHOW" :class="{'aui-dialog-in': FADE==1, 'aui-dialog-out': FADE==0}">
		<div class="aui-mask" v-if="mask" @click.stop="maskTapClose ? hide() : ''"></div>
		...
		>
			<div class="aui-dialog-title" v-if="title">{{title}}</div>
			<div class="aui-dialog-content" v-if="msg!=''" :style="{'text-align': msg.length > 15 ? 'left' : 'center'}" v-html="msg"></div>
			<div class="aui-dialog-content" v-if="items.length > 0">
				<div class="aui-dialog-input-list" v-for="(item, index) in items" :key="index" :data-index="index">
					<div class="aui-dialog-input-label" v-if="item.label">{{item.label}}</div>
					<div class="aui-dialog-input-list-input">
						<input :type="item.type ? item.type : 'text'" :value="item.value" :data-index="index"  @input="_onInput" :placeholder="item.placeholder" />
					</div>
				</div>
			</div>
			<div class="aui-dialog-down" :style="{padding: theme==4 ? (btns.length <= 1 ? '0 40px' : '0 20px') : ''}">
				<div 
